...
def main(args):
    ...
    # orthogonalize the up weights
    unet_lora_params_up_list = list(itertools.chain(*unet_lora_params_up))
    text_encoder_lora_params_up_list = list(itertools.chain(*text_encoder_lora_params_up)) if args.train_text_encoder else []
    if args.ortho_init_up:
        for param in unet_lora_params_up_list + text_encoder_lora_params_up_list:
    ...
    params_to_optimize_up = (
        [
            {"params": unet_lora_params_up_list, "lr": args.learning_rate * args.up_lr_multiplier},
            {"params": text_encoder_lora_params_up_list, "lr": text_lr * args.up_lr_multiplier},
        ]
        if args.train_text_encoder
        else unet_lora_params_up_list
    )

    optim_args['lr'] = args.learning_rate * args.up_lr_multiplier

    if args.stiefel_optim_up:
        if args.which_optim == 'adam':
            optimizer_up = StiefelAdam(
                params_to_optimize_up,
                **optim_args,
            )
            base_optimizer = StiefelAdam
        elif args.which_optim == 'sgd':
            optimizer_up = StiefelSGD(
                params_to_optimize_up,
                **optim_args,
            )
    ...
    if args.zero_init_down:
        ...
        for param in unet_lora_params_down_list + text_encoder_lora_params_down_list:
            ...
            #         rr = torch.randn(1, param.shape[1], device=param.device)
            #     param.data = param.data * rr
        print("After zero initialization")
